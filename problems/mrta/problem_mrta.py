    ...
    def __init__(self, filename=None, size=50, num_samples=1000000, offset=0,
        ...
        if filename is not None:
            ...
            for i in range(num_samples):
                ...
                agents_location = (torch.randint(0, 101, (max_n_agent, 7)).to(torch.float) / 100)

                # loc = torch.FloatTensor(size, 2).uniform_(0, 1)
                loc = torch.FloatTensor(size, 7).uniform_(0, 1)
                workload = torch.FloatTensor(size).uniform_(.2, .2)
                d_low = (((loc[:, None, :].expand((size, max_n_agent, 7)) - agents_location[None].expand(
                    (size, max_n_agent, 7))).norm(2, -1).max() / max_speed) + 20).to(torch.int64) + 1
                ...
                case_info = {
                    ...
                    'depot': torch.FloatTensor(1, 7).uniform_(0, 1),
